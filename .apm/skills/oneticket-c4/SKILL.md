---
name: oneticket-c4
description: "Generate C4 architecture documentation using Mermaid diagrams. Writes to how/c4/ in the project docs_path. Use for architecture diagrams, system context, container, component, deployment, or dynamic diagrams."
version: "2.0.0"
---
...
Architecture documentation represents a technical vision and implementation trajectory.
Do not model only the current repository state when the architecture knowledge defines a target runtime or future version.
When needed, label diagrams or notes with status such as `current`, `planned`, or `open`.
Preserve V1/V2 distinctions and open architecture questions instead of flattening them into a single current-state diagram.
